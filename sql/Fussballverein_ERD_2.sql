...
CREATE TABLE obmann ( -- df: mult=1.0
 ...
 PRIMARY KEY (name,persnr,sid),

 FOREIGN KEY (name,sid) REFERENCES fanclub (name,sid),
...
CREATE TABLE spielt ( -- df: mult=1.0
 nummer INT NOT NULL, -- df: pattern='[a-z]{999}'
 persnr INT NOT NULL, -- df: nogen
...
DROP TABLE absolviert CASCADE CONSTRAINTS;
CREATE TABLE absolviert ( -- df: mult=1.0
 bezeichnung VARCHAR(10) NOT NULL,
 persnr INT NOT NULL,
...
CREATE TABLE spiel ( -- df: mult=1.0
 datum timestamp(10) NOT NULL PRIMARY KEY, -- df: timestamp
 mannschaft VARCHAR(255) NOT NULL, -- df: pattern='(SV/BV)'
 gegner VARCHAR(255) NOT NULL, -- df: pattern='[a-z]{2,5}'
 ergebnis VARCHAR(10) NOT NULL, -- df: pattern='[0-6]{1} : [0-6]{1}'
 bezeichnung VARCHAR(10) NOT NULL,
 persnr INT NOT NULL,

 FOREIGN KEY (bezeichnung,persnr) REFERENCES absolviert (bezeichnung,persnr)
);
...
CREATE TABLE beteiligt ( -- df: mult=1.0
 ...
 dauer DECIMAL(10), -- df: pattern='PENIS'
